...
# Title: REDSAT receiver (UDP)
# Author: Christoph Honal, Alexander Ulanowski
# Generated: Wed Dec 26 20:44:33 2018
##################################################
...
class receiver_gui_udp(gr.top_block, Qt.QWidget):

    def __init__(self, config_file='./default.ini', meta_rec_udp_port=7474):
        gr.top_block.__init__(self, "REDSAT receiver (UDP)")
        Qt.QWidget.__init__(self)
        self.setWindowTitle("REDSAT receiver (UDP)")
        ...
        self.settings = Qt.QSettings("GNU Radio", "receiver_gui_udp")
        ...
        self.blocks_udp_source_0 = blocks.udp_source(gr.sizeof_gr_complex*1, '0.0.0.0', meta_rec_udp_port, 1472, True)
        ...
        self.connect((self.blocks_udp_source_0, 0), (self.time_updater, 0))
...
def argument_parser():
    ...
    parser.add_option(
        "", "--meta-rec-udp-port", dest="meta_rec_udp_port", type="intx", default=7474,
        help="Set meta_rec_udp_port [default=%default]")
    return parser


def main(top_block_cls=receiver_gui_udp, options=None):
    if options is None:
    ...
    tb = top_block_cls(config_file=options.config_file, meta_rec_udp_port=options.meta_rec_udp_port)
    tb.start()
    tb.show()
